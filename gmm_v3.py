...
X_old = None
Y_old = None
B_mean_estimate = None
R_mean_estimate = None

# Function to calculate GMM and find the new location of the agents using a predetermined scaling factor 
def estimateMeanConvergence(noMoveClick, moveClick, noOfResets, alpha, noOfSamples):
    ...
    global X_old
    global Y_old
    global B_mean_estimate
    global R_mean_estimate
    ...
    # Calculate distance between old and new means 
    if i!=0:
        R_new_mean_estimate, B_new_mean_estimate = tuple(gmm.means_[0]), tuple(gmm.means_[1])
        dst_B = distance.euclidean(B_new_mean_estimate,B_mean_estimate)
        dst_R = distance.euclidean(R_new_mean_estimate,R_mean_estimate)
        print("Distance between means: ",dst_B,dst_R)
        # B_mean_estimate, R_mean_estimate = B_new_mean_estimate, R_new_mean_estimate
        # If distance is high switch labels
                
        if (dst_B > 5) or (dst_R > 5):
            print("switch")
            labels = abs(labels-1)
            control = 1
    ...
    if control == 1:
        ...
        R_mean_estimate, B_mean_estimate = tuple(gmm.means_[0]), tuple(gmm.means_[1])
...
# Dash app components
app.layout = html.Div([
    html.H3(id='title', children = 'Tracking Dynamic Distributions: Label Switching in Gaussian Mixture Modeling'),
    dcc.Graph(
...
# Invoke the callbacks from user input  
@app.callback(
...
# All inputs from the callback are passed into this function
def generateData(noMoveClick, moveClick, noOfResets, alpha, submitClicks, noOfSamples):
    print("App Inputs: noMoveClick, moveClick, noOfResets, alpha, submitClicks, noOfSamples")
    print(noMoveClick, moveClick, noOfResets, alpha, submitClicks, noOfSamples)
    estimateMeanConvergence(noMoveClick, moveClick, noOfResets, alpha, noOfSamples)
